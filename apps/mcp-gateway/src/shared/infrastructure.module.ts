...
import { CONFIG, loadConfig, type McpGatewayConfig } from '../config/index.js';
...
@Module({
  providers: [
    { provide: CONFIG, useFactory: (): McpGatewayConfig => loadConfig() },
    {
      provide: MongoClient,
      useFactory: async (config: McpGatewayConfig): Promise<MongoClient> => {
    ...
    {
      provide: Db,
      useFactory: (client: MongoClient, config: McpGatewayConfig): Db =>
        client.db(config.MONGO_DATABASE),
      inject: [MongoClient, CONFIG],
    },
    {
      provide: Redis,
      useFactory: (config: McpGatewayConfig): Redis =>
    ...
    },
    {
      ...
      useFactory: (config: McpGatewayConfig): JwtVerifier =>
        new JwtVerifier({
