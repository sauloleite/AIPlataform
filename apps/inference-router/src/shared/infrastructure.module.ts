...
import { CONFIG, loadConfig, type RouterConfig } from '../config/index.js';
...
@Module({
  providers: [
    { provide: CONFIG, useFactory: (): RouterConfig => loadConfig() },
    {
      provide: MongoClient,
      useFactory: async (config: RouterConfig): Promise<MongoClient> => {
    ...
    {
      provide: Db,
      useFactory: (client: MongoClient, config: RouterConfig): Db =>
    ...
    },
    {
      ...
      useFactory: (config: RouterConfig): JwtVerifier =>
        new JwtVerifier({
