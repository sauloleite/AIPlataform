...
import { CONFIG, loadConfig, type RegistryConfig } from '../config/index.js';
...
@Module({
  providers: [
    { provide: CONFIG, useFactory: (): RegistryConfig => loadConfig() },
    {
      provide: MongoClient,
      useFactory: async (config: RegistryConfig): Promise<MongoClient> => {
    ...
    {
      provide: Db,
      useFactory: (client: MongoClient, config: RegistryConfig): Db =>
        client.db(config.MONGO_DATABASE),
      inject: [MongoClient, CONFIG],
    },
    {
      provide: Redis,
      useFactory: (config: RegistryConfig): Redis =>
    ...
    },
    {
      ...
      useFactory: (config: RegistryConfig): JwtVerifier =>
        new JwtVerifier({
