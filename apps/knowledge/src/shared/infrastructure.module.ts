...
import { CONFIG, loadConfig, type KnowledgeConfig } from '../config/index.js';
...
@Module({
  providers: [
    { provide: CONFIG, useFactory: (): KnowledgeConfig => loadConfig() },
    {
      provide: MongoClient,
      useFactory: async (config: KnowledgeConfig): Promise<MongoClient> => {
    ...
    {
      provide: Db,
      useFactory: (client: MongoClient, config: KnowledgeConfig): Db =>
        client.db(config.MONGO_DATABASE),
      inject: [MongoClient, CONFIG],
    },
    {
      provide: Redis,
      useFactory: (config: KnowledgeConfig): Redis =>
    ...
    },
    {
      ...
      useFactory: (config: KnowledgeConfig): JwtVerifier =>
        new JwtVerifier({
